...
function getWeeksArr(wksArr, yrIndex) {
    return wksArr.map((val, valIndex) => {
        ...
        const dotStyle = currWeek == finalWeek.value - 1 ? " death" : currWeek < weekNum.value ? " solid" : currWeek == weekNum.value ? " pulse" : ""
    ...
    <section id="calendar">
        ...
        <div class="dozen-grid">
            <div class="year-grid" v-for="(year, yearIndex) in yearsArr" :key="yearIndex">
                <div class="dot" v-for="(week, weekIndex) in getWeeksArr(weeksArr, yearIndex)" :key="weekIndex"
                    :class="week.dotStyle">
